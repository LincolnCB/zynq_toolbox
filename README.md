...
- `vivado_gui`: Open the built Vivado project (`project.xpr`) in the interactive Vivado GUI for inspection or manual Tcl testing, building `xpr` first if needed (it doesn't build any SD-card output). How the GUI is displayed depends on your `MODE` -- see the mode-specific notes in the [VM](documentation/vm_install.md#opening-the-vivado-gui) or [Docker](documentation/docker_install.md#opening-the-vivado-gui) install instructions.
...
This example project covers the Direct Memory Access (DMA) interface, which allows the PS to transfer data to and from the PL through the off-chip DDR memory, using an AXI MCDMA with DDR-backed FIFO buffers. This is the approach the Rev D Shim firmware uses to give its shallow PL FIFOs a large backing store.
