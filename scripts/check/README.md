...
```
Check every module directory in `projects/<project_name>/kernel_modules/`, verifying that each is named in kebab-case and contains a `petalinux/` folder with a `Makefile` and a matching `<module>.c`. Does nothing if the project has no `kernel_modules/` directory.
